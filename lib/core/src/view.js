...
class View {
    ...
    buildBinding(node, type, declaration, binder, identifier) {
        const parsedDeclaration = parsers_1.parseDeclaration(declaration);
        const keypath = parsedDeclaration.keypath;
        const pipes = parsedDeclaration.pipes;
        this.bindings.push(new binding_1.Binding(this, node, type, keypath, binder, pipes, identifier));
    }
    /**
    ...
    build() {
        ...
        for (i = 0, len = elements.length; i < len; i++) {
            ...
            parsers_1.parseNode(this, elements[i], this.options.templateDelimiters);
        }
                ...
                if (attribute.name.indexOf(bindingPrefix) === 0) {
                    ...
                    if (binder === null) {
                        if (this.options.binders.hasOwnProperty('*')) {
                            ...
                            binder = riba_1.Riba.fallbackBinder;
                        }
                    }
...
exports.View = View;
View.debug = debug_module_1.Debug('riba:view');
View.DECLARATION_SPLIT = /((?:'[^']*')*(?:(?:[^\|']*(?:'[^']*')+[^\|']*)+|[^\|]+))|^$/g;
/**
...
View.bindingComparator = (a, b) => {
    const aPriority = a.binder ? (a.binder.priority || 0) : 0;
    const bPriority = b.binder ? (b.binder.priority || 0) : 0;
