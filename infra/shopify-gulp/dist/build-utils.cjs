...
gulp_1.default.task("compress", () => {
    const distFiles = `${config_cjs_1.config.dist.root}**/*`;
    const ignoreConfig = `!${config_cjs_1.config.dist.root}config.yml`;
    return gulp_1.default
        .src([distFiles, ignoreConfig])
        .pipe((0, gulp_plumber_1.default)(utilities_cjs_1.errorHandler))
        .pipe((0, gulp_zip_1.default)(`${config_cjs_1.config.packageJson.name}_${config_cjs_1.config.packageJson.version}.zip` ||
        "theme.zip"))
        .pipe((0, gulp_size_1.default)({ showFiles: true, pretty: true }))
        .pipe(gulp_1.default.dest("./upload/"));
});
//# sourceMappingURL=build-utils.cjs.map