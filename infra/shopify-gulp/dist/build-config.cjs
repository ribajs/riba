...
const processConfig = (file) => {
    messages_cjs_1.default.logProcessFiles("build:config");
...
gulp_1.default.task("build:config", () => {
    return processConfig(config_cjs_1.config.tkConfig);
});
/**
...
gulp_1.default.task("watch:config", () => {
    return chokidar_1.default
        .watch(config_cjs_1.config.tkConfig, { ignoreInitial: true })
        .on("all", (event, path) => {
...
gulp_1.default.task("watch:dist-config", () => {
    return chokidar_1.default
        .watch(config_cjs_1.config.dist.root + config_cjs_1.config.tkConfig, { ignoreInitial: true })
        .on("all", (event, path) => {
        messages_cjs_1.default.logFileEvent(event, path);
        throw new Error(messages_cjs_1.default.configChange());
    });
});
//# sourceMappingURL=build-config.cjs.map