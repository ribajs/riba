...
function processSnippetIcons(files) {
    messages_cjs_1.default.logProcessFiles("build:svg");
    return gulp_1.default
        .src(files, { nodir: true })
        .pipe((0, gulp_plumber_1.default)(utilities_cjs_1.errorHandler))
        .pipe((0, gulp_svgmin_1.default)(config_cjs_1.config.plugins.svgmin)) // TODO
        .pipe((0, gulp_cheerio_1.default)(config_cjs_1.config.plugins.cheerio))
        ...
        .pipe(gulp_1.default.dest(config_cjs_1.config.dist.snippets));
}
/**
...
const removeSnippetIcons = (files) => __awaiter(void 0, void 0, void 0, function* () {
    messages_cjs_1.default.logProcessFiles("remove:svg");
    const mapFiles = files.map((file) => {
        gulp_util_1.default.log("remove icon: " + file);
        const distFile = file
            .replace(config_cjs_1.config.src.iconset, "dist/snippets")
            .replace(config_cjs_1.config.ribaShopify.root, config_cjs_1.config.dist.root);
...
const removeAssetIcons = (files) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const mapFiles = files.map((file) => {
        gulp_util_1.default.log("remove icon: " + file);
        const distFile = file
            .replace(config_cjs_1.config.src.iconset, "dist/assets")
            .replace(config_cjs_1.config.ribaShopify.root, config_cjs_1.config.dist.root);
        // TODO rename basename to iconset_ + basename?
        return distFile;
    });
...
gulp_1.default.task("build:svg:snippet", () => {
    return processSnippetIcons([
        config_cjs_1.config.src.iconset,
        config_cjs_1.config.ribaShopify.src.iconset,
    ]);
});
/**
...
gulp_1.default.task("watch:svg:snippet", () => {
    const cache = (0, utilities_cjs_1.createEventCache)();
    return chokidar_1.default
        .watch([config_cjs_1.config.src.iconset, config_cjs_1.config.ribaShopify.src.iconset], {
        ignoreInitial: true,
    })
        .on("all", (event, path) => {
        messages_cjs_1.default.logFileEvent(event, path);
        cache.addEvent(event, path);
        (0, utilities_cjs_1.processCache)(cache, processSnippetIcons, removeSnippetIcons);
    });
});
/**
...
gulp_1.default.task("watch:svg:asset", () => {
    ...
    return chokidar_1.default
        ...
        messages_cjs_1.default.logFileEvent(event, path);
        cache.addEvent(event, path);
        (0, utilities_cjs_1.processCache)(cache, processAssetIcons, removeAssetIcons);
    });
});
//# sourceMappingURL=build-svg.cjs.map