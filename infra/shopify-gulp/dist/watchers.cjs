...
const environment = config_cjs_1.config.environment.split(/\s*,\s*|\s+/)[0];
...
deploy = function async(cmd, files, env) {
    messages_cjs_1.default.logChildProcess(cmd);
    activeDeploy = true;
    ...
    return themekit_1.default
        ...
        .then(() => {
        activeDeploy = false;
        fs_1.default.appendFileSync(config_cjs_1.config.deployLog, messages_cjs_1.default.logDeploys(cmd, files)); // eslint-disable-line no-sync
        return checkDeployStatus();
    })
        .catch((err) => {
        activeDeploy = false;
        messages_cjs_1.default.logTransferFailed(err);
        fs_1.default.appendFileSync(config_cjs_1.config.deployLog, messages_cjs_1.default.logDeployErrors(cmd, files, err)); // eslint-disable-line no-sync
        return checkDeployStatus();
    });
};
/**
...
//# sourceMappingURL=watchers.cjs.map