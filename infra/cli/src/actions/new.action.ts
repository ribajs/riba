...
export class NewAction extends AbstractAction {
   ...
   * @param options
   */
  protected async generateExampleFiles(
    inputs: CommandInput[],
    options: CommandInput[]
  ) {
    ...
    const generateComponent = await this.getInputsForGenerateExamples(
    ...
    await generateComponentAction.handle(
      generateComponent.inputs,
      generateComponent.options
    );

    const generateBinderAction = new GenerateAction();
    const generateBinder = await this.getInputsForGenerateExamples(
      projectNameInput.value,
      inputs,
      options,
      generateBinderAction,
      "binder"
    );
    this.setInput(generateBinder.options, "flat", true);
    await generateBinderAction.handle(
      generateBinder.inputs,
      generateBinder.options
    );

    const generateFormatterAction = new GenerateAction();
    const generateFormatter = await this.getInputsForGenerateExamples(
      projectNameInput.value,
      inputs,
      options,
      generateFormatterAction,
      "formatter"
    );
    this.setInput(generateFormatter.options, "flat", true);
    await generateFormatterAction.handle(
      generateFormatter.inputs,
      generateFormatter.options
    );
  }
   ...
   * @param schematic
   */
  private async getInputsForGenerateExamples(
    name: string,
    inputs: CommandInput[],
    options: CommandInput[],
    ...
    const pathInput = await generateAction.setPathInput(
      clonedInputs,
      clonedOptions,
      configuration,
      schematicInput
    ...
    this.setInput(
      clonedInputs,
      "path",
      join(applicationSourceRoot, pathInput.value)
    );
