...
export const searchForTsIndexPath = (
  ...
  tsSourceDir: Config["tsSourceDir"] = DEF.TS_SOURCE_DIR,
  distFilename?: string
) => {
  let searchFor = distFilename
    ? [distFilename]
    : ["main.ts", "csr.ts", "app.ts", "index.ts"];
...
export const searchForScssIndexPath = (
  template: Config["template"] = DEF.TEMPLATE,
  scssSourceDir: Config["scssSourceDir"] = DEF.SCSS_SOURCE_DIR
) => {
...
export const parseConfig = async (config: Partial<Config>): Promise<Config> => {
  ...
  config.scssIndexPath =
    config.scssIndexPath ||
    searchForScssIndexPath(config.template, config.tsSourceDir);

  config.templateDir = config.templateDir || DEF.TEMPLATE_DIR;

  config.distDir = config.distDir || (await getDefaultDistDir(config.template));
  config.distFilename =
    config.distFilename || getDefaultDistFilename(config.tsIndexPath);
  config.distPath =
    config.distPath || resolve(config.distDir, config.distFilename);
