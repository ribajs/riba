...
describe('each-*', () => {
    ...
    it('reflects changes to the model into the DOM after unbind/bind', () => {
        const view = riba.bind(fragment, model);
        (fragment.childNodes[1].textContent as string).should.be.exactly('0');

        view.unbind();
        view.bind();
        model.items[0].val = 'howdy';
        (fragment.childNodes[1].textContent as string).should.be.exactly('howdy');
    });
    ...
    it('lets you pop an item', () => {
        ...
        ((fragment.childNodes[2] as any)._key).should.exactly(2);
    });
