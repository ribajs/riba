...
export class GenerateAction extends AbstractAction {
  ...
  protected async setDefaults(inputs: CommandInput[], options: CommandInput[]) {
    ...
    this.setDefaultInput(
      options,
      "flat",
      configuration[schematicInput.value].flat
    );

    const pathInput = await this.setPathInput(
      inputs,
      options,
      configuration,
      schematicInput
    ...
    inputs.forEach((input) => {
      if (input.name !== "schematic" && input.value !== undefined) {
        this.schematicOptions.push(
          new SchematicOption(input.name, input.value)
        );
      }
    });
  ...
  public async setPathInput(
    ...
    if (!pathInput || typeof pathInput.value !== "string") {
      ...
      if (
        typeof schematicInput.value === "string" &&
        configuration[schematicInput.value] &&
        configuration[schematicInput.value].path
      ) {
        const currentDir = fsr.getDirname();
        const targetDir = fsr.getDirname(
          configuration[schematicInput.value].path
        );
        if (currentDir !== targetDir) {
          pathInput = this.setInput(
            inputs,
            "path",
            join(
              sourceRootOption.value,
              configuration[schematicInput.value].path
            )
          );
        }
